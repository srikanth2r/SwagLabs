...
describe('Login Page Suite', () => {
    ...
    it('User Logins and Selects Products ', async () => {
        ...
        await expect(Homepage.products()).toExist();
        ...
        /*
        await cartCheck(Homepage.btnaddtocartsaucelabsbackpack, '1');   
        await cartCheck(Homepage.btnaddtocartsaucelabsbikelight, '2');
        */
...
/*
async function cartCheck(element: ChainablePromiseElement<Promise<WebdriverIO.Element>>, count: string) {
    element.click();
    browser.pause(100000)
    let cart1 = await getCartValue();
    console.log("cart value: " + cart1)
    await expect(cart1).toMatch(count)
    browser.takeScreenshot();
}


async function getCartValue(): Promise<string> {
    browser.pause(1000000)
    let value = await Homepage.shoppingcartbadge.getText();
    return value;
